...
export function errorPanelSvg(file: string, diagnostics: Diagnostic[]): string {
  const allErrors = diagnostics.filter((d) => d.severity === "error");
  const errors = allErrors.slice(0, 10);
  const more = allErrors.length - errors.length;
...
<text x="28" y="62" font-size="11" fill="#7a5a5a">the diagram will refresh as soon as the file compiles again</text>\n`;
  errors.forEach((err, index) => {
    ...
    out += `<text x="28" y="${rowY}" font-size="12.5" fill="#c53030" font-weight="bold">${esc(err.code)}</text>
<text x="92" y="${rowY}" font-size="12.5" fill="#333">line ${err.span.line}: ${esc(err.message)}</text>\n`;
    if (err.help)
      out += `<text x="92" y="${rowY + 17}" font-size="11" fill="#666">help: ${esc(err.help.length > 90 ? err.help.slice(0, 90) + "\u2026" : err.help)}</text>\n`;
  });
  if (more > 0)
