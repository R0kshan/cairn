...
export const explanations: Record<string, string> = {
  ...
  E0226:
    "`size:` is a container's knob. A leaf box is sized by the label in it — the uniform node width, or the tighter fit `label-padding:` asks for — so a delta there would argue with the one thing that decides it. A container has no such measure of its own: it is whatever holds its children, which is the size a resize handle has something to say about. Use `label-padding:` in `style` to change how tightly every box hugs its text, or `offset:` to move this one.",
  W0575:
