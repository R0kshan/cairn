 ...
 * because simple-icons does not: `license.url` is populated only for `custom`
 * licences, and every icon cairn vendors declares an SPDX id instead (verified
 * against 16.29.0). A rendered SVG has no `licenses/` beside it, so an absolute
 * URL is the only link it can carry.
 *
 * Publisher URLs where there is one. MIT and BSD-3-Clause are templates with no
 * single publisher, so those point at SPDX and the holder's own copyright line
 * travels separately — see `copyrightLineOf`.
 */
const LICENSE_URLS = {
 ...
 * ships rather than retyped, because MIT and BSD-3-Clause ask for the notice
 * itself and a link to a template does not supply it.
 *
 * Null when the shipped text carries no real line — `licenses/BSD-3-Clause.txt`
 * keeps SPDX's `<year> <owner>` because the OpenJDK artwork publishes no
 * copyright notice anywhere reachable (see SOURCE_NOTES).
 */
function copyrightLineOf(licenseType) {
  ...
  // A placeholder is not a copyright line: BSD-3-Clause leaves `<year> <owner>`,
  // Apache-2.0's appendix shows `[yyyy] [name of copyright owner]`. Neither names
  // a holder. A filled-in notice states a year, a placeholder spells one — so
  // test for the year, which also keeps the holder's own `<email>` intact.
...
const SOURCE_NOTES = {
  openjdk:
    ...
    "vector/Agent.svg>; that Mercurial host is retired and answers 403, so the " +
    "attribution above names the live GitHub location of the same file. Checked " +
    "2026-09-04: `openjdk/duke` has no LICENSE file, the GitHub API reports no " +
    "licence for it, and `vector/Agent.svg` carries no copyright notice. With no " +
    "upstream line to reproduce, `licenses/BSD-3-Clause.txt` keeps the SPDX " +
    "`<year> <owner>` fields blank and attribution is to the source URL.",
};

/**
 * Live replacements for a recorded `source` that has since died. Attribution for
 * these marks *is* the URL — no copyright line stands behind them — and a
 * rendered SVG now carries it, so a dead link ships inside every diagram drawing
 * the logo.
 *
 * An override is a verified claim: check the replacement serves the same artwork,
 * and record what upstream says in SOURCE_NOTES so the substitution stays visible.
 */
...
try {
  ...
  for (const slug of [...CURATED].sort()) {
    ...
    if (license && PERMITTED_LICENSES.has(license.type) && license.type !== "CC0-1.0") {
      ...
      // The notice table can cite a relative path; an exported SVG travels alone
      // and can only carry an absolute URL. Missing one ships a diagram whose
      // attribution names a licence it cannot point at.
    ...
    // An override replaces a dead URL, never a missing one — with no recorded
    // source there is nothing to have verified it against.
  ...
  // Only icons with their own terms get the extra fields: CC0-1.0 covers the
  // rest and asks for no attribution, so `license: null` would cost bytes in
