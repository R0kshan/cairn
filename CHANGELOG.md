...
### Changed
- A flow the router has no shape for is now offered a corridor clear of the drawing. `optimiseRoutes` builds candidates per side pair — a plain L for a mixed pair, Zs for opposing sides, a channel U for same-facing ones — and for a mixed pair the L turns at one seat's own coordinate, which is often the one line it cannot take. On `logical-security`, *Run a study* leaves the research analyst eastward and arrives at the statistical report from the north, and every L and Z between those faces either strikes a container's name or hugs the boxes under it. With nothing to take, the flow kept what elk drew: a corridor 3px above *Cohort building* and *Health-data anonymisation*, crossing *Share a report* twice and running near-parallel to it — five tier-2 defects at once, and no shape in the generator to answer them. It now goes over the top of *Secondary use* and down into the report's own north face, carrying one extra turn in place of all five. `approachLanes` already built that corridor and was reachable only by a **pinned** flow, on the grounds that an unpinned one has the whole side matrix to search; it is now reachable by any flow that matrix has failed. Narrow, because the ladder is the wrong bar for it: a four-turn detour is tier 3 and a hug or a crossing tier 2, so the ladder takes that trade every time and the corpus says readers do not. Offered alongside the ordinary shapes it put `attachAway` 386 → 483 and `turnHeavy` 860 → 967, both through their ceilings, across 66 drawings. So it is generated only after every side pair and every seat offset has been tried and refused, it must clear **three** tier-2-or-worse defects to be worth its turns, and it may not seat a terminal on the face looking away from its counterpart — a corridor leaves by a side of its own choosing, so it can always find one, and that guard alone took `attachAway` from +9.0 to +0.4 per 1000 flows. With all three: every corpus ceiling holds, no drawing regresses, 14 improve, and `crossings` falls 2.0 per 1000 flows
- A drawing is now seated against the left margin instead of keeping the strip beside it. `fitCanvas` only ever grows the canvas right and down, so nothing ever pulled a layout *back*: an element that ended up inset — or that an author dragged inward — left the space it vacated behind for good. 109 of the 352 swept drawings carried such a strip, `logical-helios-fr` 647px of it, and `large/page` came out 2564px wide to hold 2421px of drawing. The same shift answers the other direction, which only ran when the diagram carried a hint: five drawings sat at a negative `minX`, with geometry outside the viewBox for the renderer to clip. Nothing is drawn left of the margin now, and nothing is drawn needlessly far from it. A translation is the one move that cannot cost a defect — every node, run and label keeps its position relative to every other — and the ladder is unchanged on all five tiers, which is what the corpus measures. The bands are drawn from the canvas, so the legend follows the new left edge and lines up with the drawing above it. It is also the one move that leaves an author's `offset:` saying what it said: the deltas are applied first (§17) and the shift carries what they produced along with everything else, so the same source renders the same picture and a written-back offset stays valid
- Two flows leaving the same node side now nest instead of crossing. The sibling-seat swap has always answered the crossing such a pair makes close to the node, by exchanging their seats — but where the two then run out to *parallel lanes*, the seat is only half the order, and exchanging it alone leaves the lanes inverted and the crossing exactly where it was. On `small/tall`, *Search a slot and book* left the patient's west face **above** *Appointment confirmation* and then descended **outside** it, so it had to cut through that flow's riser 200px down the page; the swap saw its own candidate still crossing and declined. The lane now travels with the seat — the higher seat takes the outer lane — and the drawing comes out with no crossings at all. Offered as a second shape, never a replacement, so every existing swap is decided exactly as before, and it clears the same guards: the mutual crossing must go, no third flow may gain one, no new hug, no coincident run, no seat collision. One guard is new and applies only to the moved lane: it may not come to rest along another flow's words. A lane is a long run carried across the drawing, `readability.ts` has no straddle predicate to weigh one with, and without the check `logical-archi` picked up four straddles and `application-tech-stack-large-dense` three. Charging the plain seat swap for it as well refused swaps that have been clearing crossings all along, at a cost of 0.602 crossings per 1000 flows
- The router no longer refuses a route because a riser is shorter than the label it carries. A label lies *across* the line it names, so a riser needs to be as long as the label is **tall**, not as long as it is wide — `label-anchor` has always measured it that way, and `readability.ts`, the profile `optimiseRoutes` weighs candidates with, measured the width on both axes. So any route whose only seat was such a riser scored `unlabelled`, tier 1, and was refused however much it cleared below it (INVARIANTS §3a: a gate and the pass it drives may not disagree). On `small/tall` that is the whole reason *Appointment confirmed* left the scheduler's east face and cut across *Open / block slots* — leaving by the bottom face straight into the notifier was available the whole time and scored unlabelled, for a 113px label on a 77px riser it fits across perfectly well. It now leaves downward and *Open / block slots* comes in from the right; neither meets the other. Corpus-wide this is worth `crossings` -7.8, `attachAway` -6.8 and `labelStraddled` -6.4 per 1000 flows, with every ladder tier improving. Two conditions come with the relaxation, both measured: the riser must be the route's longest run, since a 24px corner stub otherwise "holds" a 116px label on the same arithmetic as a 400px one and `small`'s F04 swung its words back across *External systems*' border; and the sideways overhang may not land on another flow's words or lines, which the old width test prevented by accident — without it eight labels came to rest on a foreign run, `labelPierced`, a must-be-zero
- A repaired corridor no longer runs under the words of the flow it passes. On `small` (every disposition), *Open / block slots* left the medical secretary's east side and ran straight through *Appointment confirmation*'s riser, hopping over it — and the obvious alternative, the lane below, was where that flow's own 52px label already sat. Two blind spots kept it there: the lane search counted nodes, containers and container titles as obstacles but not flow labels, so the band the label allocator had already reserved looked empty; and `readability.ts`, the profile the route repair weighs candidates with, carries no straddle predicate at all, so a corridor laid along a neighbour's text scored as free. The lane search now clears foreign labels lying along the corridor, and the corridor generator drops a candidate that straddles one. The flow now leaves the secretary's bottom, takes the corridor below *Appointment confirmation*'s, and climbs into the scheduler's bottom — the crossing is gone. Both halves are narrow by measurement: adding the straddle predicate to the shared profile instead traded 57 straddles for 87 crossings across 71 drawings; counting labels seated on runs *across* the corridor, rather than along it, cost seven drawings their ratchets for a strike a few pixels wide; and the candidate is dropped only while another survives, since labels are re-anchored after this pass and vetoing outright put `labelStraddled` up 2.8 per 1000 flows
- A sibling-seat swap may no longer land a flow on top of a third one. `swapCrossingSiblingSeats` checked the pair it was moving against hugs and against each other, but not against the rest of the drawing — `readability.ts` rates a coincident run tier 0 and calls it `merge:`, and nothing in that pass consulted it. Surfaced when the corridor work above gave the swap new seats to choose from, and it put ten runs under other runs, a must-be-zero breach
- The route-repair audit now counts the crossings a revert would bring back. "Crossings are the router's business, already weighed" holds for the repair, which the router chose, and not at all for the reverted state, which nothing weighed — so a revert could quietly restore a crossing the repair had cleared
- `compact` is now asserted on the *boxes* rather than on the canvas. The style governs spacing between nodes; the canvas is whatever `fitCanvas` grows to once the router has laid its corridors, and on `small` the two now disagree — compact packs the boxes tighter (1136x194 → 1084x185) and still ends up the taller canvas, because its corridor clears a label band 26px further down than the same corridor does at normal spacing
