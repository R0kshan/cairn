...
| 4d. Anchor | [`label-anchor.ts`](../src/label-anchor.ts) | `Scene` → `Scene` | Seats every flow label on its own run. At the *final* anchor only (the one that moves labels and nothing else — every pass below the mid-pipeline anchor routes on where labels are), prefers a seat whose text band clears every container outline, reaching past a cramped run's end when that run has only one seat, and never moving more than 24px from the seat it would otherwise pick |
