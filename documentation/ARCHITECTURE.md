...
| 4d. Anchor | [`label-anchor.ts`](../src/label-anchor.ts) | `Scene` → `Scene` | Seats every flow label on its own run. At the *final* anchor only (the one that moves labels and nothing else — every pass below it routes on where labels are), prefers a seat whose text band clears every container outline, reaching past a cramped run's end when that run has only one seat, and never moving more than 24px from the seat it would otherwise pick. Where no seat is clear, `preferClearBorders` runs after collision resolution and slides the label along its own run to the seat crossing fewest outlines — counted, not merely detected, and never onto a node or another label |
| 4e. Air | [`scene-layout.ts`](../src/scene-layout.ts) | `Scene` → `Scene` | `airOutContainers` pushes a container border outward up to 3px where a flow crossing it would otherwise have its arrowhead drawn against the frame. Borders only — no node and no route moves, which is why it can run after every routing pass and why it costs none of the re-routing that widening elk's `elk.padding` does. Clamped by the container that holds it, by other boxes, and by any run lying outside the border |
