...
| 4c. Compact | [`compact.ts`](../src/compact.ts) | `Scene` → `Scene` | Removes only bands with zero pinning content (no node, label, or segment running across them) — never reintroduces an overlap. `compactVertical` reclaims horizontal bands on every disposition and never distorts a container; `compactHorizontal` reclaims columns in the reading direction only (`wide`/`slide`), and there a container *is* narrowed when the dead columns sit inside it — its borders and title still pin, so no box ends up narrower than what it holds. Also owns `fitCanvas`, which grows the canvas to hold every node, route point and label with a 10px margin — grow-only, so a drawing that already fits is untouched |
