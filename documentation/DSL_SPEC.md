...
| `infrastructure` | `server` → `network-zone` / `site`; `app-instance` → `server` / `network-zone`; `network-zone` → `site` / `network-zone`; `gateway` / `firewall` / `auth` / `idp` → `network-zone` / `site` (convention, not enforced) |
